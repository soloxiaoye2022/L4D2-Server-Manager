...
select_plugins_tui_uninstall() {
    # TUI 配置
    local title="插件卸载管理器"
    local instruction="[↑/↓]移动 [空格]选中/取消 [V]查看文件 [Enter]确认卸载"
    local total=${#subfolde[@]}
    local cursor=0
    local start_idx=0
    local page_size=10
    ...
    while true; do
        clear
        echo -e "\e[41;37m $title \e[0m"
        echo -e "\e[33m$instruction\e[0m"
        echo "----------------------------------------"
        
        # 计算显示范围
        if [ $cursor -lt $start_idx ]; then
            start_idx=$cursor
        elif [ $cursor -ge $((start_idx + page_size)) ]; then
            start_idx=$((cursor - page_size + 1))
        fi
        ...
        for ((i=start_idx; i<end_idx; i++)); do
            ...
            # 检查安装状态
            local status_str="\e[90m[未安装]\e[0m"
            if [[ -d "${DEFAULT_SH}/steamcmd/${DEFAULT_DIR}/left4dead2/addons/${folder_name}" ]]; then
                status_str="\e[32m[已安装]\e[0m"
            fi
            
            if [ $i -eq $cursor ]; then
                echo -e "\e[7m $prefix $folder_name $status_str \e[0m"
            else
                echo -e " $prefix $folder_name $status_str"
            fi
        done
        
        echo "----------------------------------------"
        echo -e "当前选择: $cursor / $((total-1)) (总计: $total)"
        ...
        case "$key" in
            ...
            " ") # Space
                ...
                ;;
            "v"|"V") # View files
                local current_plugin="${subfolders[cursor]}"
                if [ -d "$current_plugin" ]; then
                    clear
                    echo -e "\e[32m=== 插件文件结构预览: $(basename "$current_plugin") ===\e[0m"
                    if command -v tree >/dev/null 2>&1; then
                        tree -N "$current_plugin" | head -n 30
                        if [ $(find "$current_plugin" | wc -l) -gt 30 ]; then
                            echo "... (内容过长，仅显示前30行)"
                        fi
                    else
                        ls -R "$current_plugin" | head -n 30
                    fi
                    echo -e "\e[33m按任意键返回列表...\e[0m"
                    read -n 1 -s -r
                fi
...
plugins_unload_tui() {
    ...
    # 2. 调用卸载TUI
    if ! select_plugins_tui_uninstall; then
        return
    fi
    ...
    for folder_name in "${selected_folders[@]}"; do
         # 查找源路径
         local src_path=""
         for p in "${subfolders[@]}"; do
             if [ "$(basename "$p")" == "$folder_name" ]; then
                 src_path="$p"
                 break
             fi
         done
         ...
         if [ -n "$src_path" ]; then
             ...
             while IFS= read -r -d '' file; do
                 local rel_path="${file#./}"
                 local dest_file="$server_root/$rel_path"
                 if [ -f "$dest_file" ]; then
                     rm -f "$dest_file"
                     ((file_count++))
                 fi
             done < <(cd "$src_path" && find . -type f -print0)
             
             # 尝试删除对应的空目录（如果存在插件同名目录）
             if [ -d "$server_root/addons/$folder_name" ]; then
                 rm -rf "$server_root/addons/$folder_name"
             fi
             
             echo -e "\e[46;34m卸载完成 (移除文件数: $file_count)\e[0m"
         else
             echo -e "\e[31m源文件丢失，跳过: $folder_name\e[0m"
         fi
    done
    
    echo -e "\e[32m所有选定插件卸载操作已完成。\e[0m"
    echo -e "\e[33m按回车键返回主菜单...\e[0m"
    read -r
}
